const useFluidCursor = () => {
  ...
  // Site palette: accent violet / cyan / pink (matches --accent tokens).
  // Every splat picks from these — the fluid reads as a cohesive brand
  // glow instead of a random rainbow.
  const PALETTE = [
    [0.486, 0.424, 1.0],    // #7c6cff
    [0.302, 0.839, 1.0],    // #4dd6ff
  ...
  return {
    // Exposed for the cursor integration: tint the active pointer's
    // color + emit a splat (so a main.js hook can punch a burst).
    splatColor: (r, g, b) => {
      pointers.forEach((p) => { p.color = [r * 0.15, g * 0.15, b * 0.15]; });
    },
    burst: (x, y, force) => {
      const p = pointers[0];
      p.prevTexcoordX = p.texcoordX = x / canvas.width;
      p.prevTexcoordY = p.texcoordY = 1.0 - y / canvas.height;
      p.deltaX = force;
      p.deltaY = force;
      p.moved = true;
      splatPointer(p);
